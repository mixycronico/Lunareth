...
from corec.serialization import serializar_mensaje
...
async def procesar_entidad(entidad: MicroCeluEntidadCoreC, umbral: float = 0.5) -> bytes:
    ...
    if not activo:
        return await serializar_mensaje(
            int(id_[1:]) if id_.startswith("m") else int(id_),
            canal, 0.0, False
        )
    try:
...
async def procesar_celu_entidad(entidad: CeluEntidadCoreC, datos: Dict[str, Any], umbral: float = 0.5) -> bytes:
    ...
    if not activo:
        return await serializar_mensaje(
            int(id_[1:]) if id_.startswith("c") else int(id_),
            canal, 0.0, False
        )
    try:
