        ...
        if modulo_registro:
            ...
            if bloque_1 and bloque_2:
                nuevo_bloque = BloqueSimbiotico(
                    nuevo_id,
                    bloque_1.canal,
                    bloque_1.entidades[:500] + bloque_2.entidades[:500],
                    max_size=1024,
                ...
                self.logger.info(
                    f"Bloques {bloque_id_1} y {bloque_id_2} fusionados en {nuevo_id}"
                )
            else:
                self.logger.error(
                    f"No se encontraron bloques {bloque_id_1} o {bloque_id_2}"
                )
        ...
        if modulo_registro:
            ...
            if bloque:
                ...
                if carga > 0.8 and num_entidades > 500:
                    ...
                    modulo_registro.bloques[nuevo_id] = nuevo_bloque
                    self.logger.info(
                        f"Bloque {bloque_id} dividido en {nuevo_id}"
                    )
                ...
                if modulo_registro:
                    for bloque_id, bloque in modulo_registro.bloques.items():
                        if bloque.fitness < 0.2:
                            candidato = next(
                                (
                                    b for b_id, b in modulo_registro.bloques.items()
                                    if b.canal == bloque.canal and
                                    b_id != bloque_id and b.fitness > 0.5
                                ),
                                None
                            )
                            if candidato:
                                nuevo_id = (
                                    f"fused_{bloque_id}_{time.time_ns()}"
                                )
                                await self.fusionar_bloques(
                                    bloque_id, candidato.id, nuevo_id
                                )
