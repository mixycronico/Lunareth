...
class ModuloRegistro(ModuloBase):
    ...
    async def inicializar(self, nucleus):
        ...
        for bloque_config in self.nucleus.config.get("bloques", []):
            await self.registrar_bloque(
                bloque_config["id"],
                bloque_config["canal"],
                bloque_config["entidades"]
            )

    async def registrar_bloque(self, bloque_id: str, canal: int, num_entidades: int):
        entidades_por_bloque = 1000
        num_bloques = (
            (num_entidades // entidades_por_bloque) +
            (1 if num_entidades % entidades_por_bloque else 0)
        )
        ...
        for j in range(num_bloques):
            ...
            for i in range(
                min(entidades_por_bloque, num_entidades - j * entidades_por_bloque)
            ):
                async def funcion():
                    return {"valor": random.random()}

                entidades.append(
                    crear_entidad(f"m{i + j * entidades_por_bloque}", canal, funcion)
                )

            bloque = BloqueSimbiotico(
                bloque_id_j, canal, entidades, nucleus=self.nucleus
            )
            self.bloques[bloque_id_j] = bloque
            self.logger.info(
                f"Bloque {bloque_id_j} registrado con {len(entidades)} entidades"
            )
    ...
    async def registrar_celu_entidad(self, celu: CeluEntidadCoreC):
        ...
        self.logger.info(
            f"CeluEntidad {celu.id} registrada en bloque {bloque_id}"
        )

    async def registrar_micro_celu_entidad(self, micro: MicroCeluEntidadCoreC):
        bloque_id = f"micro_{micro.id}"
        bloque = BloqueSimbiotico(
            bloque_id, micro.canal, [micro], nucleus=self.nucleus
        )
        self.bloques[bloque_id] = bloque
        self.logger.info(
            f"MicroCeluEntidad {micro.id} registrada en bloque {bloque_id}"
        )
