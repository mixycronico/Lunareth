    ...
    def ejecutar_bloque(bloque_id: str, db_config: Dict[str, Any], instance_id: str):
        ...
        try:
            ...
            try:
                cur = conn.cursor()
                cur.execute(
                    "SELECT canal, num_entidades FROM bloques WHERE id = %s",
                    (bloque_id,)
                )
                result = cur.fetchone()
                if result:
                    ...
                    bloque = BloqueSimbiotico(
                        bloque_id, canal, [], max_size=1024
                    )
                    asyncio.run(bloque.escribir_postgresql(db_config))
                    logging.getLogger("ModuloEjecucion").info(
                        f"Bloque {bloque_id} ejecutado"
                    )
                else:
                    logging.getLogger("ModuloEjecucion").error(
                        f"Bloque {bloque_id} no encontrado"
                    )
                cur.close()
            finally:
                conn.close()
            ...
            logging.getLogger("ModuloEjecucion").error(
                f"Error ejecutando bloque {bloque_id}: {e}"
            )
            raise
            ...
            try:
                ...
                if modulo_registro:
                    for bloque_id in modulo_registro.bloques.keys():
                        self.ejecutar_bloque.delay(
                            bloque_id, self.nucleus.db_config, self.nucleus.instance_id
                        )
