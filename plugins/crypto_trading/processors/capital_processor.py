...
class CapitalProcessor:
    def __init__(self, config, redis_client, trading_db, nucleus):
        ...
        self.base_capital = 100  # Capital base inicial
    ...
    def actualizar_total_capital(self, nuevo_total: float):
        ...
        # Simular consenso para ajustar el capital base
        if self.total_capital > self.base_capital * 2:
            self.base_capital *= 2  # Duplicar el capital base
            self.logger.info(f"Consenso simulado: Capital base ajustado a ${self.base_capital}")
        elif self.total_capital < self.base_capital / 2:
            self.base_capital /= 2  # Reducir a la mitad
            self.logger.info(f"Consenso simulado: Capital base ajustado a ${self.base_capital}")
